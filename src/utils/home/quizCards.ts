export function quizCards() {
  ...
  const visibleIconCards = document.querySelectorAll('.quiz_small-card_features-icon-true');

  visibleIconCards.forEach((card) => {
    // Find the corresponding false icon in the same parent element
    const falseIcon = card.parentElement?.querySelector('.quiz_small-card_features-icon-false');
